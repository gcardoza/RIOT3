...
void loop()
{
  ...
  if(Current_Time >= Last_Publish_Time + Update_Interval) Publish_Irrigation_Status();
...
void Turn_Zone_On(int zone)
{
  ...
  if (zone == 0) Serial.println("   -> Turning off Active_Zone");
  else
  {
  ...
  Publish_Irrigation_Status();        // Send Status to BioT that a new Zone is On
}
